...
#then corner coords, down to the grid indeces

#load scene_with_FF_UTM_correct_row_cols.csv

scene_data <- read_csv("scene_with_FF_UTM_correct_row_cols.csv")
...
cornercalc <- function(arow){
  row <- unlist(scene_data[arow,'true_row'])
  col <- unlist(scene_data[arow,'true_col'])
...
rows <- nrow(scene_data)
sceneslice_coords <- sapply(1:nrow(scene_data), cornercalc)
