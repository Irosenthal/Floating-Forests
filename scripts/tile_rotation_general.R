...
scenelookup <- filter(full_subjects, scene_ID == "LE70440351999204EDC01")
...
library(stringr)
subjects_correct <- read_csv("./corrected_subjects_full.csv")
subjects_correct <- subjects_correct %>%
  mutate(sceneID = str_sub(metadata.file, end = -8))

scenelookup <- filter(subjects_correct, sceneID == "LE70440351999204EDC01")
#LE70440351999204EDC01
#LC80400372013110LGN01
scenelookup <- scenelookup %>%
  mutate(bad_rows_adjusted = bad_rows +1) %>%
  mutate(bad_cols_adjusted = bad_cols +1)


scene_with_FF_UTM <- scenelookup %>%
  rowwise() %>%
  mutate(tile_ID = true_tiles[bad_rows_adjusted, bad_cols_adjusted])
